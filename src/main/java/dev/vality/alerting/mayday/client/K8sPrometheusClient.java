...
import static dev.vality.alerting.mayday.client.error.ErrorCodes.NOT_FOUND_ERROR_CODE;
...
public class K8sPrometheusClient {
    ...
    public void deletePrometheusRule(PrometheusRule prometheusRule) throws KubernetesClientException {
        try (KubernetesClient client = new KubernetesClientBuilder().withConfig(k8sConfig).build()) {
            ...
            try {
                prometheusRuleClient.resource(prometheusRule).delete();
            } catch (KubernetesClientException e) {
                if (!e.getStatus().getCode().equals(NOT_FOUND_ERROR_CODE)) {
                    throw e;
                }
                log.warn("Tried to delete non existing rule", e);
            }
        }
    }
